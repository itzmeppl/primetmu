...
function Message() {
    ...
    return (
        <div className="Message">
            ...
            />
            <button onClick={() => sendMessage(tempCurRoom)}> Send Message</button>
            <h1>{tempCurRoom}</h1>
            <div id="rooms" style={{display: 'flex', flexDirection: 'row', overflow: "auto", height: "50px" }}>
                {userRooms && userRooms
                .map((room, index) => (
                    <button type="button" value={room} onClick={() => fetchRooms(room)}>{room}</button>
                ))}
            </div>
            <div id="messages" style={{ overflow: "auto", height: "300px" }}>
                {messages && messages
                .map((msg, index) => (
                    <p style={{ paddingTop: "0px", paddingLeft: "10px", paddingRight: "10px",color: msg.author === user? 'yellow':'red', textAlign: msg.author === user? 'right':'left'}} 
